<?php
/**
 * Clase de conexión PDO - mysql.inc.php
 * Usuario BD: app_lab2fa (privilegios mínimos - NO superusuario)
...
class mod_db
{
    ...
    public function __construct()
    {
        ...
        $sql_pass = "Lab2FA_2026#Secure";  // NO es el superusuario root
    ...
    public function disconnect()       { $this->conexion = null; }
    ...
    public function updateSeguro($tabla, $data, $condiciones)
    {
        ...
        $where = [];
        foreach ($condiciones as $key => $value) { $where[] = "$key = :cond_$key"; }
        ...
        try {
            ...
            foreach ($condiciones as $key => $value) { $stmt->bindValue(":cond_$key", $value); }
            return $stmt->execute();
        } catch (PDOException $e) {
    ...
    public function del($tb_name, $astriction)
    {
        ...
        if ($astriction) { $sql .= " WHERE $astriction"; }
    ...
    {
        if ($string) { $stmt = $this->executeQuery($string); }
        if ($stmt != null) {
    ...
    public function objects($stmt = "") { return $stmt ? $stmt->fetch(PDO::FETCH_OBJ) : null; }

    public function Arreglos($string = "")
    {
        try {
            if ($string) { $stmt = $this->conexion->query($string); }
        } catch (PDOException $e) {
            echo "Error: " . $e->getMessage();
        }
    ...
    {
        return $this->conexion->lastInsertId(); // CORREGIDO: era $this->Conexion (mayúscula)
    }
}
